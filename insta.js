...
let postsData = [
      ...
      likeButton.addEventListener('click', () => {
        if (!likedPosts.has(post.id)) {
            ...
            for(let ind of likedPosts){
                ...
                //So, to get the correct button for a post with ID ind, you need to access index ind-1 in the NodeList. 
                // For example, for post ID 1, 
                // you want the button at index 0; for post ID 2, the button at index 1, and so on.
  ...
  function likePost(postId) {
    const post = postsData.find(post => post.id === postId);//this checks if the post exists
    if (post) {
      post.likes++;
      renderPosts();
    }
